        ...
        <div className={`relative flex-1 flex flex-col max-w-xs w-full bg-white transform transition ease-in-out duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
            <div className="flex-shrink-0 flex items-center px-4">
              <h1 className="text-xl font-bold text-blue-600">CheckInn Manager</h1>
            </div>
            <nav className="mt-5 px-2 space-y-1">
              {navigation.map((item) => (
                <Link
                  ...
                  onClick={() => setSidebarOpen(false)}
                >
                  <item.icon className={`${item.current ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'} mr-4 flex-shrink-0 h-6 w-6`} />
                  {item.name}
                </Link>
              ))}
            </nav>
          ...
          <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-700">{user?.name || 'Hotel Manager'}</p>
                <button
          ...
          <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
            ...
            <nav className="mt-5 flex-1 px-2 bg-white space-y-1">
              {navigation.map((item) => (
                ...
                >
                  <item.icon className={`${item.current ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'} mr-3 flex-shrink-0 h-6 w-6`} />
                  {item.name}
                </Link>
              ))}
            </nav>
